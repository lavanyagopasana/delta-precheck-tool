...
class DeltaPhaseTest {
    ...
    void preDeltaLabelsCarryTheCycleNumberAndPhase() {
        assertThat(DeltaType.PRE_DELTA.labelWithPhase("1.1", DeltaPhase.IN_APPROVAL)).isEqualTo("Pre-Delta 1.1 - in progress");
        assertThat(DeltaType.PRE_DELTA.labelWithPhase("1.1", DeltaPhase.READY)).isEqualTo("Pre-Delta 1.1 - ready to start");
        assertThat(DeltaType.PRE_DELTA.labelWithPhase("1.1", DeltaPhase.STARTED)).isEqualTo("Pre-Delta 1.1 - started");
        assertThat(DeltaType.PRE_DELTA.labelWithPhase("2.1", DeltaPhase.FINISHED)).isEqualTo("Pre-Delta 2.1 - done");
    }
    ...
    void finalDeltaCarriesEveryPhaseJustLikeAPreDelta() {
        ...
        assertThat(DeltaType.FINAL_DELTA.labelWithPhase("3.1", DeltaPhase.IN_APPROVAL)).isEqualTo("Final Delta - in progress");
        assertThat(DeltaType.FINAL_DELTA.labelWithPhase("3.1", DeltaPhase.FINISHED)).isEqualTo("Final Delta - done");
    }

    @Test
    void finalDeltaIsNeverNumbered() {
        // There is only one Final Delta per combination, so a number would be noise.
        assertThat(DeltaType.FINAL_DELTA.labelWithPhase("3.1", DeltaPhase.COMPLETE)).isEqualTo("Final Delta - completed");
        assertThat(DeltaType.FINAL_DELTA.labelWithPhase("3.1", DeltaPhase.STARTED)).isEqualTo("Final Delta - started");
    }
}
