...
| `PMO_AUTO_SYNC_ENABLED` | `false` | Off by default now that the Delta-phase webhook (below) is the one way projects arrive from PMO. Set `true` to also run the batch poll as a periodic reconciliation pass — same per-project logic either way, so nothing behaves differently depending on which path created a project. The admin-triggered `POST /api/pmo/sync` stays available regardless of this flag |
