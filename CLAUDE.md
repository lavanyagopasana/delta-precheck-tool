...
| `PMO_AUTO_SYNC_ENABLED` | `false` | **Off since 2026-09-03 by product decision — projects are created by hand, so nothing imports them in the background.** A project moving into Delta phase in PMO therefore no longer appears here on its own; that is intended. Was `true` from 2026-08-29 (as a reconciliation pass alongside the webhook, since the webhook is a single delivery with no retry). Set `true` to re-enable; the admin-triggered `POST /api/pmo/sync` stays available either way |
